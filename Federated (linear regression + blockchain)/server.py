            ...
            if (("data" in received_data.keys())
                ...
                if subject == "echo":
                    ...
                    if model is None:
                        ...
                        error = model.calc_accuracy(data_inputs, data_outputs,
                                                    "RMSE")
                    ...
                    try:
                        ...
                        if model is None:
                            ...
                            error = model.calc_accuracy(
                                data_inputs, data_outputs, "RMSE")
                            # In case a client sent a model to the server despite that the model error is 0.0. In this case, no need to make changes in the model.
                            if error <= 0.25:
                                ...
                                model_avg = self.model_averaging(
                                    model, cli_model)
                                last_block = blockchain.chain[-1]
                                new_block = bl.Block(
                        ...
                        error = model.calc_accuracy(data_inputs, data_outputs,
                                                    "MAE")
                        print("Error(RMSE) from {info} = {error}".format(
                            error=error, info=self.client_info))
    ...
    def run(self):
        ...
        while True:
            ...
            if status == 0:
                ...
                print(
                    "Connection Closed with {client_info} either due to inactivity for {recv_timeout} seconds or due to an error."
